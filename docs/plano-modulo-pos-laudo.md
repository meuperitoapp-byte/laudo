...
| 5 | ✅ **FEITO 05/09/2026**. `compilarEsclarecimentos` + reuso DIRETO de `ModeloLaudo`/`renderizarPdf`/`renderizarDocx` (2 mudanças aditivas neles: `CabecalhoFormal.paragrafoIntroducao` e "APRESENTAÇÃO" condicional a `modelo.apresentacao` não vazio — zero impacto no laudo principal). Pendências como lista item a item com âncora (`podeGerarSaida` amarrado pela 1ª vez). Two-pass de paginação via `bufferedPageRange()` do PDFKit (`bufferPages:true`) — sem lib nova. `gerarEsclarecimentos` grava `laudos_gerados` (`tipo='esclarecimentos'`, versão max+1, nunca sobrescreve); `marcarPosLaudoProtocolado` generaliza o protocolar pra qualquer saída de pós-laudo e grava a Nova Conclusão Vigente a partir do **snapshot já congelado** (nunca da coluna viva do ciclo — gerar e protocolar continuam dois atos distintos). Diálogo de protocolar avisa (sem bloquear) quando a versão escolhida não é a mais recente do ciclo, com a conclusão que ela gravaria. Simplificações registradas no README (nenhuma imprime `[___]`: nome/CRM/cidade padrão da perita, sem "ID da manifestação", sem "Fundamentação da repercussão" à parte, seção V sempre ausente até a fatia 9, páginas do `.docx` reusam a contagem do PDF). **Ajuste 05/09/2026**: data de assinatura deixou de ser "sempre hoje" — é a data do ATO, campo `<input type="date">` na tela de geração, pré-preenchido com hoje e editável (a Dra. pode gerar num dia e protocolar dias depois; o documento tem que levar a data que realmente aconteceu). |
| 6 | ✅ **FEITO 05/09/2026** (migration `20260907120000` + código). CRUD dos itens onde-se-lê/leia-se (`pos_laudo_retificacao_itens`, seção III) + Análise da Repercussão (seção IV) como **pergunta explícita** — 2 colunas novas em `pos_laudo_ciclos` (`retificacao_afeta_conclusao` boolean, `retificacao_justificativa` text), nunca inferida. `compilarRetificacao` (seções I–VII, reuso direto do `ModeloLaudo`/renderers, mesmo two-pass de paginação e data-do-ato da fatia 5) + `gerarRetificacao` + `marcarPosLaudoProtocolado` (já genérico). **NÃO** → gera doc; a justificativa entra **verbatim** na seção IV, seção V só existe nesse caminho (conclusão mantida integralmente). **SIM** → pendência de tom "orientação" (não "bloqueio"): ela acertou ao identificar a repercussão, o caminho é a Complementação (fatia 7). **Sem migração de dado no reroteamento** — `pos_laudo_retificacao_itens` já nasceu (fatia 0) chaveada só por `ciclo_id`, não pela saída. Beco sem saída evitado: a resposta não trava (pode voltar pra NÃO), os itens ficam salvos, e só a geração da Retificação fica indisponível enquanto SIM. `substitui_conclusao` sempre `false` (Retificação nunca cria Nova Conclusão Vigente — trava estrutural da fatia 0). **Gap fechado 07/09/2026** (migration `20260908120000`): seção I ganhou os 3 campos que faltavam — `retificacao_id_documento`, `retificacao_data_identificacao` (date), `retificacao_origem_identificacao` (CHECK perito/juizo/autor/reu/outro), em `pos_laudo_ciclos`; card `IdentificacaoRetificacaoControl` no topo do painel; entram na seção I do documento quando preenchidos (não obrigatórios). |
| 7 | ✅ **FEITO 07/09/2026** (migration `20260909120000` + código). Modelo completo (seções I–XI). Tabela nova `pos_laudo_complementacao` (1:1 com o ciclo) só pros campos específicos II–VII; seções I/IX/X **reusam** `pos_laudo_ciclos` (Registro da Demanda) + `laudo_base` + **`repercussao_laudo`/`conclusao_vigente_nova`** (o mesmo enum de 6 valores da fatia 4). É a ÚNICA saída que aceita `repercussao_laudo = 'substituicao_conclusao'` — nos Esclarecimentos essa opção virou pendência de tom "orientação" ("gere a Complementação em vez disso"). §IV (nova avaliação) e §V (exames/especialista) são condicionais por toggle. **Fecha o caminho SIM da Retificação**: `compilarComplementacao` lê `pos_laudo_retificacao_itens` do ciclo e renderiza a tabela onde-se-lê/leia-se dentro da §VI, read-only, antes do texto livre — a promessa da mensagem calma do SIM se concretizando. `compilarComplementacao` + `gerarComplementacao` + `salvarComplementacao` (upsert 1:1) + `ComplementacaoPanel` (form único, um botão "Salvar", §II–VII). Mesmo two-pass de paginação e data-do-ato das fatias 5-6. `substitui_conclusao` = `snapshot.conclusao_vigente_texto !== null`. Pendências de geração: §IX não preenchida, Nova Conclusão Vigente quando exigida, §VI fundamentação vazia, §IV/§V ligadas sem o campo-âncora. |
| 8 | ✅ **FEITO 08/09/2026** (sem migration — a fatia 0 já pôs `'encerrado'` no CHECK de `status` e a coluna `encerrado_em`). **Resposta (d) da Dra., confirmada em áudio: gerar só as saídas que fizerem sentido em cada rodada — nunca PDF único consolidado, nunca forçar as 3.** Não sobrou orquestração real além do encerramento em si (a fatia 7 já resolveu o handoff Retificação→Complementação). `encerrarCiclo` (`status='encerrado'` + `encerrado_em=now()`) / `reabrirCiclo` (volta pra `'aberto'`, limpa `encerrado_em`) — encerrar é **reversível** (≠ protocolar). Não exige nenhum documento gerado (rodada pode terminar com 0). Componente `EncerramentoCiclo` no fim da tela do ciclo: resumo das 3 saídas (versões geradas + protocolo, dos dados que a página já tem) + botão encerrar/reabrir. Com o ciclo encerrado, os 3 botões "Gerar" ficam desabilitados (aviso `CicloEncerradoAviso`); **protocolar continua liberado**; os formulários de edição ficam como estão (editar sem regerar é inócuo — documentos protocolados são snapshots congelados). |
| 9 | ✅ **FEITO 08/09/2026** (sem migration — `pos_laudo_quesitos` já existe desde a fatia 0). **Decisão (c) da Dra.: numeração REINICIA do 1 a cada ciclo** — `adicionarQuesitoCiclo` usa `max(numero) WHERE ciclo_id` + 1. `salvarQuesitoCiclo` / `removerQuesitoCiclo`. `QuesitosCicloPanel` (client, card por quesito: origem/tipo/pergunta/resposta, botão explícito, selo "Incompleto"). Helper `montarSecaoQuesitos` compartilhado renderiza a **seção V dos Esclarecimentos** e a **VIII da Complementação** (condicionais — some quando não há quesito), agrupadas por origem (autora/ré/Juízo/outros), numeradas de 1 por grupo; §VIII ganha um parágrafo de intro deixando claro que as respostas são complementares. Cada quesito com pergunta/resposta vazia vira pendência de geração (análogo a "ponto sem resposta técnica"). Snapshot `quesitos_ciclo` populado. Fica só no ciclo — **não** entra na aba Quesitos do laudo. |
| 10 | ✅ **FEITO 10/09/2026** (migration `20260910120000_pos_laudo_at.sql` + código, em 3 sub-fatias: 10a schema/tipos, 10b workspace, 10c compiladores+geração). Ativa `fluxo='assistencia_tecnica'` reusando 1-9. **Mudança de arquitetura central (resposta (b) da Dra.)**: o documento de AT NÃO é protocolado pelo sistema — enquanto `protocolado=false`, `gravarSaidaAtInPlace` SOBRESCREVE a mesma linha `laudos_gerados` (mesmo id/versão/caminho no Storage) em vez de empilhar versão nova; só quando alguém registra que o patrono protocolou (`marcarPosLaudoProtocolado`, reaproveitado) a linha congela. Estado intermediário `entregue_ao_advogado_em` (reversível, `registrarEntregaAoAdvogado`). `abrirCicloPosLaudo` no AT não exige laudo protocolado nem conclusão vigente (`laudo_base_id` null — análise avulsa, resposta (e)). Registro da demanda AT: objeto, tese, `classificacao_global` (obrigatória, cobrada na geração), providência recomendada, posição PERICONS **sem log versionado** (resposta (a)). `pos_laudo_at_analise` (tabela 1:1, 13 eixos boolean|null) = análise estruturada do laudo judicial. `MatrizPontos` ganhou `modoAt` (esconde Nova Conclusão Vigente; troca "repercussão do ponto" por `categoria_problema`). **Um compilador só** (`compilar-parecer-at.ts`) parametrizado por `at_modalidade` (6 valores → 4 `tipo`) + **documento isolado** `compilar-quesitos-at.ts` (item 7 da aprovação — quesitos saem embutidos no parecer E como peça própria, pendências deliberadamente mais leves). `BlocoQuesitos.semResposta` (mudança aditiva em `geracao-laudo/modelo.ts` + os 2 renderers) omite a linha de resposta pros quesitos AT (elaborados pro advogado, nunca respondidos aqui). |
| 11 | ✅ **FEITO 10/09/2026** (sem migration — `situacao_processo` já existe, `text` livre com catálogo fechado em `catalogos.ts`). `SituacaoProcessoSugestao` (um componente pros 2 momentos): ciclo **aberto** sugere marcar `SITUACAO_PROCESSO_POS_LAUDO` (some sozinha quando já está nela); ciclo **encerrado** sugere voltar, com `<select>` da lista fechada pra ela escolher (o sistema não decide pra qual volta). "Agora não" dispensa só na sessão de tela. `sugerirSituacaoProcesso` (actions.ts) valida contra o catálogo antes de gravar. Nunca automático — mesma regra decidida em 03/09/2026. |
| 12 | ✅ **FEITO 10/09/2026** (sem migration — só leitura sobre `laudos_gerados` + `pos_laudo_conclusoes_vigentes`). `/processos/[id]/pos-laudo/linha-do-tempo`: todas as versões do processo em ordem cronológica (`versao` monotônica = ordem de criação), com tipo/título (`titulo || TIPO_DOCUMENTO_ROTULOS[tipo]`), data, estado (Gerado / Entregue ao advogado / Protocolado) e o ciclo de origem com link ("Laudo original" quando `pos_laudo_ciclo_id` é null). Bloco de topo com a conclusão vigente atual + a partir de qual documento ela passou a valer (`origem_laudo_gerado_id`, acrescentado ao select de `conclusaoVigenteAtual`); a mesma linha na lista ganha o selo "Conclusão vigente atual". Motivo de fazer agora (Jeferson, não ficou opcional): a tela do laudo final filtra por tipo e as saídas do pós-laudo vivem dentro do ciclo — não havia lugar único pra ver tudo que já saiu do processo. Links "Ver linha do tempo" no índice de ciclos e "Ver linha do tempo completa" na tela do laudo final; `TIPO_DOCUMENTO_ROTULOS` centralizado em `rotulos.ts` (corrigiu a lacuna do `quesitos_at`, que faltava no mapa local antigo de `gerar-laudo-panel.tsx`). |
...
**Feitas: fatias 0 → 10.** Todas as 5 perguntas da Dra. foram respondidas (até
08/09/2026) e as fatias 8, 9 e 10 saíram na sequência (08-10/09/2026). O módulo cobre
Judicial (3 saídas + encerramento + quesitos) e Assistência Técnica (parecer
parametrizado + Quesitos Suplementares isolado + geração in-place) por completo.
- **Fatia 11** pode ser puxada a qualquer momento — seu único bloqueio ("fim da rodada
  sugere, não muda sozinho") foi decidido em 03/09/2026.
- **Fatia 12** (opcional) pode entrar a qualquer momento.
...
**Respondidas pela Dra. Fernanda (todas até 08/09/2026):**
- **(a) AT — "posição sobre o laudo":** **não precisa de histórico versionado** —
  basta a posição atual constar em cada documento. *(destrava fatia 10)*
- **(b) Retificação no AT:** ela **sempre refaz o parecer inteiro** (nunca correção
  pontual). **MAS:** o documento de AT **não é protocolado pelo sistema** — quem
  protocola é o advogado, externamente. Há uma janela entre ela entregar e o advogado
  protocolar de fato, durante a qual ela pode editar a mesma página e reentregar. →
  **a trava "documento protocolado = versão imutável" do Judicial NÃO vale igual na
  AT**: o documento de AT continua editável até haver confirmação (fora do sistema) de
  que o advogado protocolou. *(muda a arquitetura da fatia 10)*
- **(c) Numeração dos quesitos suplementares:** **REINICIA sempre do 1 a cada novo
  ciclo** — nunca continua a contagem do laudo original. *(destrava fatia 9)*
- **(d) "As 3 saídas juntas":** **só as aplicáveis à rodada** — nunca PDF único
  consolidado, nunca forçar as 3. *(fatia 8 já saiu com isso, 08/09/2026)*
- **(e) Análise de laudo avulsa no AT:** **pode ser avulsa** (caso nunca acompanhado
  antes) — é comum o advogado contratar só depois de ver o laudo do perito judicial
  sair ruim, pedindo especificamente impugnação e/ou quesitos suplementares. →
  precisa de tela de entrada mínima. *(entra no escopo da fatia 10)*
