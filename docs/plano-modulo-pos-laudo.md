...
|---|---|---|
| 0 | **Schema + tipos** | Migration com as 7 tabelas novas + colunas em `laudos_gerados` + regen de `database.ts` e `json-fields.ts` (snapshot como união discriminada por `tipo`). Testar: aplicar migration, inserir linhas via SQL, validar os CHECKs (`pos_laudo_conclusoes_vigentes.origem_tipo` sem `retificacao`; `not(laudos_gerados.tipo='retificacao' and substitui_conclusao)`). Sem UI. |
| 1 | **Abrir e listar ciclos (judicial)** | Aba "Pós-laudo" gated no laudo protocolado; `/pos-laudo` índice; form "abrir novo ciclo" (arquivo da intimação **opcional** — aviso de pendência visível se vazio); tela do ciclo só com "Registro da demanda". Testar: criar ciclo 1 e 2, editar registro, ver na lista, reabrir. |
| 2 | **Triagem + matriz de pontos** | Adicionar/editar/remover pontos; 1 classificação por ponto; campo de ciclo "pode modificar conclusão?"; evidências (vincular documentos existentes). Ação automática: classificar `necessidade_complementacao` liga `rascunho_complementacao` + aviso. |
| 3 | **Documentos supervenientes** | Upload (reusa `documentos` + bucket + signed URL) e `pos_laudo_documentos` com papel + classificação de impacto. Testar: doc superveniente **não** entra na contagem/tabela de documentos do laudo V1 (anti-join de 1.7). |
| 4 | **Resposta ponto a ponto + repercussão + conclusão vigente** | `resposta_tecnica` e `repercussao` por ponto; seção "Repercussão sobre o laudo" (ciclo); semente da conclusão V1; se repercussão altera/substitui → exige "Nova Conclusão Vigente" e grava no log; consulta "conclusão vigente agora"; trava de finalização. |
