...
const ALVO_POR_TIPO: Record<
  ...
  { valor: AceitouNomeacao; rotulo: string; nomeDocumento: string }
> = {
...
/**
 * Sugestão (nunca automática) de atualizar `processos.aceitou_nomeacao`
 * depois de protocolar o nº12 ou o nº13 — pedido do Jeferson (11/09/2026):
 * "se ela protocolou uma escusa, o processo não pode continuar se
 * comportando como encargo ativo, senão a régua e a Central de Prazos vão
 * cobrar providências de algo que ela devolveu ao juízo. Mas isso é mudança
 * de estado a partir de um ato formal... o sistema sugere, com o motivo
 * visível, e ela confirma. Nunca aplica sozinho." Mesmo mecanismo do
 * `SituacaoProcessoSugestao` do Pós-Laudo — "Agora não" dispensa só nesta
 * sessão de tela.
 */
export function AceitouNomeacaoSugestao({
  ...
  if (aceitouNomeacaoAtual === alvo.valor) return null;
