...
import { compilarAnaliseViabilidade } from "./compilar-pdf";
...
/**
 * Gera uma nova versão do PDF/Word da Análise de Viabilidade (§38) — mesmo
 * padrão de `gerarLaudo` (geracao-laudo/actions.ts): compila, renderiza os
 * dois formatos do MESMO modelo, sobe pro Storage (mesmo bucket, mesmo
 * padrão de nomes — o path já é escopado por `processoId`, sem colisão com
 * o laudo/pareceres do mesmo processo), grava a versão em `laudos_gerados`
 * com `tipo = 'analise_viabilidade'`. Nunca sobrescreve uma versão anterior
 * — `versao` é sempre a maior já usada (entre TODOS os tipos) pro processo + 1.
 */
export async function gerarAnaliseViabilidadePdf(processoId: string): Promise<ActionResult> {
  const resultado = await compilarAnaliseViabilidade(processoId);
  if (resultado.status === "erro") {
    return { error: resultado.mensagem };
  }

  const [ativos] = await Promise.all([buscarAtivosGlobais()]);
  ...
  const caminhoPdf = `${processoId}/analise-viabilidade-v${versao}.pdf`;
  const caminhoDocx = `${processoId}/analise-viabilidade-v${versao}.docx`;
  ...
  const insert: LaudosGeradosInsert = {
    ...
    titulo: "Análise de Viabilidade Técnico-Pericial",
