...
import { compilarQuesitosDocumento } from "./compilar-pdf";
...
/** Gera uma nova versão do PDF/Word dos Quesitos — mesmo padrão de gerarAtestadoPdf/gerarReplicaPdf. */
export async function gerarQuesitosDocumentoPdf(processoId: string, documentoId: string): Promise<ActionResult> {
  const resultado = await compilarQuesitosDocumento(processoId, documentoId);
  ...
  const caminhoPdf = `${processoId}/quesitos-v${versao}.pdf`;
  const caminhoDocx = `${processoId}/quesitos-v${versao}.docx`;
  ...
  const insert: LaudosGeradosInsert = {
    ...
    versao,
    tipo: "quesitos_parte",
    titulo: "Quesitos Técnico-Periciais",
