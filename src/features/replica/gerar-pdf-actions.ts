...
import { compilarReplica } from "./compilar-pdf";
import { TITULO_REPLICA } from "./compilar-pdf";
...
/** Gera uma nova versão do PDF/Word da Orientação para Réplica — mesmo padrão de gerarAtestadoPdf. */
export async function gerarReplicaPdf(processoId: string, replicaId: string): Promise<ActionResult> {
  const resultado = await compilarReplica(processoId, replicaId);
  ...
  const caminhoPdf = `${processoId}/replica-v${versao}.pdf`;
  const caminhoDocx = `${processoId}/replica-v${versao}.docx`;
