...
import { compilarRelatorioTecnico, TITULO_RELATORIO_TECNICO } from "./compilar-pdf";
...
/** Gera uma nova versão do PDF/Word do Relatório Técnico — mesmo padrão de gerarAtestadoPdf/gerarReplicaPdf. */
export async function gerarRelatorioTecnicoPdf(processoId: string, relatorioId: string): Promise<ActionResult> {
  const resultado = await compilarRelatorioTecnico(processoId, relatorioId);
  ...
  const caminhoPdf = `${processoId}/relatorio-tecnico-v${versao}.pdf`;
  const caminhoDocx = `${processoId}/relatorio-tecnico-v${versao}.docx`;
  ...
  const insert: LaudosGeradosInsert = {
    ...
    titulo: TITULO_RELATORIO_TECNICO,
